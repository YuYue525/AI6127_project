...
class RNN(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_dim, dropout_ratio, hidden_dim, out_dim):
        ...
        self.rnn = nn.RNN(300, hidden_dim, 3, batch_first = True)
        self.relu = nn.ReLU()
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        if validation_loss < best_valid_loss:
            ...
            torch.save(model.state_dict(), "./check_points/rnn_loss.pt")

        if validation_accuracy > best_valid_acc:
            best_valid_acc = validation_accuracy
            torch.save(model.state_dict(), "./check_points/rnn_acc.pt")
    ...
    filename = './check_points/rnn.pt'
    torch.save(model.state_dict(), filename)

    fig = plt.figure()
    plt.plot(train_loss, label = 'Training Loss')
    plt.plot(val_loss, label = 'Validation Loss')
    plt.title("train curves for RNN on SNLI")
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    filename = './img_results/rnn.png'
