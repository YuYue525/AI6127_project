...
log_file = "./log_bert_ensemble.txt"
train_loss_history = []
train_f1_history = []
...
if __name__ == '__main__':
    ...
    for i in range(NUM_MODELS):
        ...
        indices = random.choices(range(len(train_data)), k=len(train_data))
        train_data_bag = [train_data[idx] for idx in indices]
        train_dataset = SentencePairDataset(train_data_bag, tokenizer, MAX_LENGTH)
        ...
        torch.save(model.state_dict(), './bert_{}.pt'.format(i+1))
        models.append(model)
    ...
    predictions = numpy.int64(predictions)
    test_pred = numpy.array([numpy.argmax(numpy.bincount(prediction)) for prediction in predictions.T])

    test_f1_score = f1_score(target_label, test_pred, average='macro')
    test_precision = precision_score(target_label, test_pred, average='macro')
    test_recall = recall_score(target_label, test_pred, average='macro')
    test_acc = accuracy_score(target_label, test_pred)
    
    f = open(log_file, "a+")
    f.write(f'Test F1 score: {test_f1_score:.4f}, Test acc: {test_acc:.4f}, Test precision: {test_precision:.4f}, Test recall: {test_recall:.4f}\n\n')
    f.close()
