...
class BiRNN(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_dim, dropout_ratio, hidden_dim, out_dim):
        ...
        self.rnn = nn.RNN(300, hidden_dim, 3, batch_first = True, bidirectional = True)
        self.relu = nn.ReLU()
    ...
    def forward(self, batch):
        ...
        encoded_premise, _ = self.rnn(premise_projection)
        encoded_hypothesis, _ = self.rnn(hypothesis_projection)
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        if validation_loss < best_valid_loss:
            ...
            torch.save(model.state_dict(), "./check_points/birnn_loss.pt")

        if validation_accuracy > best_valid_acc:
            best_valid_acc = validation_accuracy
            torch.save(model.state_dict(), "./check_points/birnn_acc.pt")
    ...
    filename = './check_points/birnn.pt'
    torch.save(model.state_dict(), filename)

    fig = plt.figure()
    plt.plot(train_loss, label = 'Training Loss')
    plt.plot(val_loss, label = 'Validation Loss')
    plt.title("train curves for BiRNN on SNLI")
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    filename = './img_results/birnn.png'
